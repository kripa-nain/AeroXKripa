...
drone = p.loadURDF("quadrotor.urdf", dronePos)      #Loading the drone
...
while(True):
    ...
    if keycode.get(p.B3G_RETURN) == 1:        #If ENTER key is pressed then the simulation executes
        ...
        plane = p.loadURDF("plane.urdf")        #Loading the plane and drone again
        dronePos = [0,0,0.1]
        drone = p.loadURDF("quadrotor.urdf", dronePos)
        ...
        while(True):
            ...
            P = 2.55   # Your Trackbar 'P' value / 10
            I = 0.201   # Your Trackbar 'I' value / 1000
            D = 2500.0 # Your Trackbar 'D' value * 5
            # --------------------------------
